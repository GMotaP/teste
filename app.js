/* app.js · PAY4CHARGE
   Requer auth.js carregado antes (define DATA_API, loadJSONP, asArray,
   getUserEmail, logout, allowedChargers). */
...
// ---------- load ----------
async function load(){
  // Gate: sem login, não mostra nada (redireciona).
  ...
  try{
    // 1) carregadores liberados para este e-mail
    ...
    // 3) filtra apenas os carregadores deste usuário
    ...
    if(!RAW.length){
      document.getElementById("status").innerHTML =
        ...
        'O e-mail <b>'+email+'</b> não tem carregadores associados (ou ainda não há sessões para eles). '+
        'Verifique a planilha de carregadores por e-mail.</div>';
      return;
    }
    ...
    const msg = '<b>Não consegui carregar os dados.</b><br>'+e.message+
      '<br><br>Usando JSONP. As 3 APIs (dados, usuários, carregadores) precisam ter '+
      '<code>doGet(e)</code> devolvendo <code>e.parameter.callback + "(" + json + ")"</code> '+
      'com <code>MimeType.JAVASCRIPT</code>, implantação como <b>"Qualquer pessoa"</b>, '+
      'e nova implantação após editar.';
