...
function loadJSONP(url, timeoutMs=25000){
  return new Promise((resolve,reject)=>{
    ...
    const timer = setTimeout(()=>{ cleanup(); reject(new Error("Tempo esgotado ao chamar a API (JSONP).")); }, timeoutMs);
    function cleanup(){
      clearTimeout(timer);
      delete window[cb];
...
async function load(){
  try{
    ...
    const msg = '<b>Não consegui carregar os dados.</b><br>'+e.message+
      '<br><br>Usando JSONP. Confira no seu Apps Script:<br>'+
      '• o <code>doGet(e)</code> precisa devolver <code>e.parameter.callback + "(" + json + ")"</code> '+
      'com <code>MimeType.JAVASCRIPT</code> quando vier o parâmetro <code>callback</code>;<br>'+
      '• a implantação deve estar como <b>"Qualquer pessoa" (Anyone)</b>;<br>'+
      '• reimplante (Nova implantação) após editar o código para a URL refletir as mudanças.';
